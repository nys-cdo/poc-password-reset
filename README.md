...
| **Sub-path deploys** | Icons, assets, and routing must respect the deploy base path. | `setupIcons` uses `import.meta.env.BASE_URL`; the router reads a `basename` from it (see [`src/App.tsx`](src/App.tsx)); Vite `base` is set for the build **and** `vite preview` (`command === "build" \|\| isPreview`). |
| **Raw anchors ignore basename** | React Router's `basename` only rewrites its own `<Link>`. NYSDS `href`/`homepageLink` props (button, dropdown item, global header, breadcrumbs) render **raw `<a>`** → an app-relative path 404s under a sub-path deploy. | Two `useLocale` helpers: `localePath` for `<Link to>`/`navigate`, `localeHref` (base-prefixed) for raw NYSDS `href`. Details in [`docs/nysds-notes.md`](docs/nysds-notes.md) §10. |
